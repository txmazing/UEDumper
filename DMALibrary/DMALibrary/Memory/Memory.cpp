...
bool DMAMemory::DumpMemoryMap(bool debug)
{
...
bool DMAMemory::Init(std::string process_name, bool memMap, bool debug)
{
	if (!DMA_INITIALIZED)
	{
		LOG("inizializing...\n");
...
DWORD DMAMemory::GetPidFromName(std::string process_name)
{
	DWORD pid = 0;
...
std::vector<int> DMAMemory::GetPidListFromName(std::string name)
{
...
std::vector<std::string> DMAMemory::GetModuleList(std::string process_name)
{
...
VMMDLL_PROCESS_INFORMATION DMAMemory::GetProcessInformation()
{
...
size_t DMAMemory::GetBaseDaddy(std::string module_name)
{
...
size_t DMAMemory::GetBaseSize(std::string module_name)
{
	std::wstring str(module_name.begin(), module_name.end());
...
uintptr_t DMAMemory::GetExportTableAddress(std::string import, std::string process, std::string module)
{
...
uintptr_t DMAMemory::GetImportTableAddress(std::string import, std::string process, std::string module)
{
...
bool DMAMemory::DumpMemory(uintptr_t address, std::string path)
{
...
uint64_t DMAMemory::FindSignature(const char* signature, uint64_t range_start, uint64_t range_end, int PID)
{
...
bool DMAMemory::Write(uintptr_t address, void* buffer, size_t size) const
{
...
bool DMAMemory::Write(uintptr_t address, void* buffer, size_t size, int pid) const
{
...
bool DMAMemory::Read(uintptr_t address, void* buffer, size_t size) const
{
	DWORD read_size = 0;
...
bool DMAMemory::Read(uintptr_t address, void* buffer, size_t size, int pid) const
{
	DWORD read_size = 0;
...
VMMDLL_SCATTER_HANDLE DMAMemory::CreateScatterHandle() const
{
...
VMMDLL_SCATTER_HANDLE DMAMemory::CreateScatterHandle(int pid) const
{
...
void DMAMemory::CloseScatterHandle(VMMDLL_SCATTER_HANDLE handle)
{
	VMMDLL_Scatter_CloseHandle(handle);
}

void DMAMemory::AddScatterReadRequest(VMMDLL_SCATTER_HANDLE handle, uint64_t address, void* buffer, size_t size)
{
...
void DMAMemory::AddScatterWriteRequest(VMMDLL_SCATTER_HANDLE handle, uint64_t address, void* buffer, size_t size)
{
...
void DMAMemory::ExecuteReadScatter(VMMDLL_SCATTER_HANDLE handle, int pid)
{
	if (pid == 0)
		pid = current_process.PID;
...
void DMAMemory::ExecuteWriteScatter(VMMDLL_SCATTER_HANDLE handle, int pid)
{
	if (pid == 0)
		pid = current_process.PID;
